...
/*
The size of the DLL comes from a value written at the end of the shellcode, 
in between the shellcode and the DLL in the shellcode buffer. 
This is one of just 2 values which are customised in the shellcode, 
the other being the ordinal that should be called on the DLL once loaded. 
The layout of the shellcode buffer looks like this

Shellcode ( size = 0x1800 or 6144 )
DLL Size
Ordinal
DLL
*/
