...
* **DoublePulsar_structure_ping.cpp** - This program sends 4 SMB packets.  1 negotiation, 1 session setup, 1 tree connect and Trans2 SESSION_SETUP packet.  The difference between this and doublepulsar_check.cpp is that this generates the Trans2 DoublePulsar PING packet using a structure rather than hardcoded SMB hex string that I pulled from a Wannacry sample.  Another difference is that this program dynamically generates the TreeConnect packet rather than using the hardcoded one present in the Wannacry sample.  The ping packet generated from this will tell you if DoublePulsar is installed on the remote machine.
